...
/* =============================================
   VOCABULARY EXERCISES — 4º (Practice por categoría
...
function renderVocabExGridC4() {
  loadVocabData(function() {
    var grid = document.getElementById('vocab-ex-topics-grid-c4');
    if (!grid) return;
    grid.innerHTML = '';

    // Tarjeta Mix, siempre primero
    var mixCard = document.createElement('div');
    mixCard.style.cssText = 'background:#F5F3FF;border:1.5px solid #DDD6FE;border-radius:16px;padding:18px 16px;display:flex;align-items:center;gap:14px;cursor:pointer;margin:0 16px 12px;box-shadow:0 2px 8px rgba(0,0,0,.06);transition:box-shadow .2s';
    mixCard.innerHTML =
      '<div style="font-size:44px">🎲</div>' +
      '<div style="flex:1">' +
        '<div style="font-family:var(--f);font-weight:900;font-size:16px;color:#7C3AED">Mix — All topics</div>' +
        '<div style="font-size:12px;color:var(--gray-400);margin-top:3px">A bit of everything</div>' +
      '</div>' +
      '<div style="font-size:20px;color:#DDD6FE">›</div>';
    mixCard.addEventListener('mouseenter', function() { mixCard.style.boxShadow = '0 4px 16px rgba(0,0,0,.12)'; });
    mixCard.addEventListener('mouseleave', function() { mixCard.style.boxShadow = '0 2px 8px rgba(0,0,0,.06)'; });
    mixCard.addEventListener('click', function() { startVocabPracticeC4(null); });
    ...
    SubjectData.vocab.units.forEach(function(unit) {
      ...
      card.addEventListener('mouseenter', function() { card.style.boxShadow = '0 4px 16px rgba(0,0,0,.12)'; });
      card.addEventListener('mouseleave', function() { card.style.boxShadow = '0 2px 8px rgba(0,0,0,.06)'; });
      card.addEventListener('click', function() { startVocabPracticeC4(unit); });
      grid.appendChild(card);
    });
  });
}

/* ---- Elegir categoría (o Mix con unit=null) → pantalla de tipo ---- */
function startVocabPracticeC4(unit) {
  _vocabPracticeUnitC4 = unit;
  setEl('vocab-ex-type-title-c4', unit ? unit.title : 'Mix — All topics');
  go('s-english-vocab-ex-type-c4');
}

function _vocabWordsC4() {
  return _vocabPracticeUnitC4 ? _vocabPracticeUnitC4.words : _vocabGetAllWords();
}

function _vocabBaseConfigC4(prefix) {
  return {
    queue:       EN.vocabExQueue,
    idx:         EN.vocabExIdx,
    prefix:      prefix,
    ...
    getAllWords: _vocabGetAllWords,
    setIdx:      function(v){ EN.vocabExIdx = v; },
    onFinish:    function(){ go('s-english-vocab-ex-c4'); },
    onAdvance:   function(mode){
      if (mode === 'word-to-image' || mode === 'w2i') loadW2IQuestionC4();
      else loadI2WQuestionC4();
    }
  };
}

function startVocabExerciseC4(type) {
  loadVocabData(function() {
    var words = _vocabWordsC4();
    EN.vocabExQueue = _vocabShuffle(words);
    EN.vocabExIdx  = 0;
    EN.vocabExType = type;
    if (type === 'word-to-image') { go('s-vocab-ex-w2i-c4'); loadW2IQuestionC4(); }
    else { go('s-vocab-ex-i2w-c4'); loadI2WQuestionC4(); }
  });
}

function loadW2IQuestionC4() {
  vocabExInit(_vocabBaseConfigC4('w2ic4'), 'word-to-image');
}

function loadI2WQuestionC4() {
  vocabExInit(_vocabBaseConfigC4('i2wc4'), 'image-to-word');
}

function checkVocabI2WC4() { vocabCheckI2W(); }
function nextVocabExC4(mode) { vocabExNext(mode); }
