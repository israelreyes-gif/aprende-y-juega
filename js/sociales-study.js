/* =============================================
   SOCIALES-STUDY.JS — Estudio con acordeón
   ============================================= */
...
function renderSocialesMenu() {
  loadSocialesData(function() {
    ...
    if (!grid) return;
    grid.innerHTML = '';
    SubjectData.sociales.units.forEach(function(unit) {
...
function renderSocialesUnit() {
  ...
  if (tabsEl) {
    ...
    SO.unit.sections.forEach(function(sec) {
      ...
      btn.style.cssText = 'padding:5px 12px;border-radius:20px;border:0.5px solid var(--gray-200);font-family:var(--f);font-size:12px;cursor:pointer;transition:all .15s;white-space:nowrap;' +
        (sec.id === SO.section
          ? 'background:' + SO.unit.color + ';color:white;border-color:' + SO.unit.color
  ...
  var sec = SO.unit.sections.find(function(s) { return s.id === SO.section; });
  if (!sec) return;
  ...
  sec.items.forEach(function(item) {
    var isOpen = SO.openItem === item.id;
    ...
    btn.innerHTML =
      '<div style="display:flex;align-items:center;gap:12px">' +
        '<div style="width:40px;height:40px;border-radius:10px;background:' + SO.unit.bg + ';display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:22px">' +
          item.icon +
        '</div>' +
        ...
        '</div>' +
      '</div>' +
      '<i class="ti ' + (isOpen ? 'ti-chevron-up' : 'ti-chevron-down') + '" style="font-size:18px;color:' + SO.unit.color + ';flex-shrink:0;margin-left:8px"></i>';
    btn.addEventListener('click', (function(id) {
      return function() {
        SO.openItem = SO.openItem === id ? null : id;
        SO.openSub = null;
        renderSocialesUnit();
      };
    })(item.id));
    ...
    if (isOpen) {
      ...
      item.subs.forEach(function(sub) {
        ...
        var isSubOpen = SO.openSub === subId;
        ...
        subBtn.innerHTML =
          '<div style="display:flex;align-items:center;gap:8px">' +
            '<div style="width:28px;height:28px;border-radius:8px;background:' + SO.unit.bg + ';display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px">' +
              sub.icon +
            '</div>' +
        ...
        subBtn.addEventListener('click', (function(sid) {
          return function() {
            SO.openSub = SO.openSub === sid ? null : sid;
            renderSocialesUnit();
          };
        })(subId));
