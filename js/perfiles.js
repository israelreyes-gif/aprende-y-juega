...
// Usar los mismos arrays que avatar.js (índices)
var SKIN_COUNT      = 5;  // AVATAR_SKINS.length
var HAIR_COLOR_COUNT = 3; // solo los 3 desbloqueados por defecto
var HAIR_NAMES = ['Liso corto','Liso largo','Rizado','Trenzas','Cola alta'];

// Estado temporal del modal (índices)
...
/* ---- Construir objeto av para drawAvatarSVG ---- */
...
function renderModalOpciones() {
  ...
  AVATAR_SKINS.forEach(function(c, i) {
    ...
    btn.style.cssText = 'width:28px;height:28px;border-radius:50%;background:'+c+';border:none;cursor:pointer;' +
      'outline:'+(modalAV.skin===i?'2.5px solid #FF6B9D':'2px solid transparent')+';outline-offset:2px;transition:outline .1s';
    btn.addEventListener('click', function() { modalAV.skin = i; renderModalOpciones(); renderModalPreview(); });
    skinRow.appendChild(btn);
  });

  // Color pelo (solo los 3 básicos desbloqueados)
  var hairRow = document.getElementById('modal-hair-row');
  hairRow.innerHTML = '';
  AVATAR_HAIR_COLORS.slice(0, 3).forEach(function(h, i) {
    var btn = document.createElement('button');
    btn.style.cssText = 'width:28px;height:28px;border-radius:50%;background:'+h.color+';border:none;cursor:pointer;' +
      'outline:'+(modalAV.hairColor===i?'2.5px solid #FF6B9D':'2px solid transparent')+';outline-offset:2px;transition:outline .1s';
  ...
  HAIR_NAMES.forEach(function(n, i) {
    var btn = document.createElement('button');
    btn.textContent = n;
    btn.style.cssText = 'padding:5px 11px;border-radius:20px;border:none;cursor:pointer;font-family:var(--f);font-weight:700;font-size:11px;transition:all .15s;' +
      'background:'+(modalAV.hair===i?'#FF6B9D':'rgba(255,255,255,.1)')+';' +
      'color:'+(modalAV.hair===i?'#fff':'rgba(255,255,255,.55)');
  ...
  if (perfilEditandoId) {
    perfiles = perfiles.map(function(p) {
      if (p.id === perfilEditandoId) {
        return { id: p.id, nombre: nombre, skin: modalAV.skin, hairColor: modalAV.hairColor, hair: modalAV.hair, unlocked: p.unlocked || [] };
      }
      return p;
    });
  } else {
